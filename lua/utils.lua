...
function _M.search_ancestors(startpath, func)
  validate { func = {func, 'f'} }
  if func(startpath) then return startpath end
  for path in _M.path.iterate_parents(startpath) do
    if func(path) then return path end
  end
end

function _M.root_pattern(...)
  local patterns = vim.tbl_flatten {...}
  local function matcher(path)
    for _, pattern in ipairs(patterns) do
      if _M.path.exists(_M.path.join(path, pattern)) then
        return path
      end
    end
  end
  return function(startpath)
    return _M.search_ancestors(startpath, matcher)
  end
end
